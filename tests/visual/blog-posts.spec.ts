...
test.describe("Blog Posts Visual Regression", () => {
  ...
  test.describe("Example Post with Assets", () => {
    ...
    test("example with assets lightbox - desktop", async ({ page }) => {
      await page.setViewportSize({ width: 1920, height: 1080 });
      await page.goto("/blog/example-with-assets");
      ...
      const trigger = page.locator(".blog-image-lightbox__trigger").first();
      const popover = page.locator(".blog-image-lightbox__popover").first();

      await trigger.click();
      await expect(popover).toBeVisible();

      await expect(page).toHaveScreenshot(
        "example-with-assets-lightbox-open-desktop.png",
      );
    ...
    test("example with assets lightbox - mobile", async ({ page }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      await page.goto("/blog/example-with-assets");
      ...
      await expect(page).toHaveScreenshot(
        "example-with-assets-lightbox-open-mobile.png",
      );
